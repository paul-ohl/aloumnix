...
export function EventList({
  ...
  const fetchEvents = useCallback(async () => {
    ...
    if ("error" in result && result.error) {
      ...
      const items = result.items as SerializedEvent[];
  ...
  const handleUpdateEvent = async (data: EventCreationInput) => {
    if (!selectedEvent) return;

    setIsPending(true);
    setUpdateError(null);

    const result = await updateEventAction(selectedEvent.id, data);
  ...
  const handleBulkDelete = async () => {
    ...
    const result = await deleteEventsAction(idsArray);
  ...
  if (error) {
    return <EventError message={error} />;
  }

  const allOnPageSelected =
    events.length > 0 && events.every((event) => selectedIds.has(event.id));
  const someOnPageSelected =
    events.length > 0 && events.some((event) => selectedIds.has(event.id));
          ...
          <div className="grid gap-4">
            {events.map((event) => (
              <EventCard
                ...
                onEdit={(e) => handleOpenModal(e, "edit")}
                onDelete={(id) => {
                  setDeleteId(id);
                  setOpenMenuId(null);
                }}
          ...
          >
            <div className="text-center">
              ...
              <p className="text-zinc-600 dark:text-zinc-400 mb-8">
                {deleteId === "bulk"
                  ? "Are you sure you want to permanently remove the selected events? This action cannot be undone."
